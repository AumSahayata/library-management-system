...
const getBookData = async (borrowed) => {
  try {
    const res = await fetch(
      `https://www.googleapis.com/books/v1/volumes?q=isbn:${borrowed[0].book_isbn}&key=${API_KEY}`
    );
    const data = res.json();
    return data;
  } catch (error) {
...
export default async function UserProfile({ data, borrowed }) {
  const borrowBooks = await getBookData(borrowed)
